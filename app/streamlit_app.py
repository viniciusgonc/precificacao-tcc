...
def renderizar_tokens_sidebar(container) -> None:
    with container:
        ...
        custo_estimado = tokens * 0.00000059  # Groq Llama 3.3 70B Versatile ~US$0,59 por 1M tokens
...
def mensagens_para_api(messages: list) -> list:
    """
    Prepara o histórico da API para envio à Groq.
...
async def maybe_summarize_history(groq_client: AsyncGroq) -> None:
    """
...
async def query_agent(prompt: str) -> None:
    ...
    with st.chat_message("assistant"):
        ...
        if not os.getenv("GROQ_API_KEY"):
            status_box.error("⚠️ Informe a GROQ_API_KEY na barra lateral ou no arquivo .env.")
            return
        ...
        try:
            async with stdio_client(server_params) as (read, write):
                async with ClientSession(read, write) as session:
                    ...
                    groq_tools = _converter_tools_mcp(tools_response.tools)
                    ...
                    # ── Loop de agente ─────────────────────────────────────────
                    while True:
                        # Usa messages_api (possivelmente resumido) para envio à Groq
                    ...
                    await maybe_summarize_history(groq_client)
...
st.caption("Consultoria inteligente com ferramentas de cálculo exato via MCP + Groq/Llama")
